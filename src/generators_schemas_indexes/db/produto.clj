(ns generators-schemas-indexes.db.produto
  (:use [clojure pprint])
...
(defn busca-mais-caro
  [db]
  (d/q '[:find (max ?preco) .
         :where [_ :produto/preco ?preco]]
    db))

(defn busca-mais-caros-que
  [db preco-minimo]
  (d/q '[:find ?preco
         :in $ ?minimo
         :where [_ :produto/preco ?preco]
...
(defn busca-por-preco
  [db preco]
  (db.entidade/datomic-para-entidade (d/q '[:find (pull ?produto [*])
                                            :in $ ?buscado
