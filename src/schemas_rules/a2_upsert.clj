...
(db/adiciona-ou-altera-produtos! conn [(assoc dama :produto/slug "/jogo_de_dama")])
(pprint (db/um-produto (d/db conn) (:produto/id dama)))
...
(defn atualiza-slug
  ...
  (let [produto (db/um-produto (d/db conn) (:produto/id dama))]
    (Thread/sleep 3000)
    (let [produto (assoc produto :produto/slug "/jogo_de_dama_caro")]
      (db/adiciona-ou-altera-produtos! conn [produto])
...
(defn atualiza-slug-inteligente
  []
  (println "Atualizando slug")
  (let [produto {:produto/id (:produto/id dama) :produto/slug "/dama_com_slug_novo"}]
    (Thread/sleep 3000)
    (db/adiciona-ou-altera-produtos! conn [produto])
    (println "Slug atualizado")
    produto))
